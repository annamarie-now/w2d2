...
function renderPrice() {
  ...
  Object.keys(ingredients).forEach(function(key) {
    if (state[key]) {
      // add price of ingredient to totalPrice
      // display ingredient in the price list
      const ingredientPrice = ingredients[key].price;
      totalPrice += ingredientPrice;

      const li = document.createElement("li");
      //   li.innerText = `$${ingredientPrice} ${ingredients[
      //     key
      //   ].name.toLowerCase()}`;
      ...
      //   innerHTML += `<li>$${ingredientPrice} ${ingredients[
      //     key
      //   ].name.toLowerCase()}</li>`;
    }
  });
